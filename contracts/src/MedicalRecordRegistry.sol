...
contract MedicalRecordRegistry is IMedicalRecordRegistry {
    ...
    /// @notice Trusted Oracle address that signs verified ZK-TLS proofs
    ...
    function setTrustedOracle(address _oracle) external onlyDeployer {
    ...
    function registerRecord(
        ...
        require(
            bytes(ipfsHash)[0] == "Q" || bytes(ipfsHash)[0] == "b",
            "Invalid CID format"
        );
    ...
    function submitVerifiedData(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(proofHash, providerId));
        bytes32 ethSignedMessageHash = MessageHashUtils.toEthSignedMessageHash(messageHash);
        require(
            ECDSA.recover(ethSignedMessageHash, oracleSignature) == trustedOracle,
